...
We can, hence, calculate the probability of the whole sequence is using the chain rule as follows:  
...
When using CNNs for speech recognition, speech signals are transformed into spectrograms (images) to organize speech features into feature maps that are suitable for CNNs. Assuming input feature maps are all one dimentsional, each unit of one feature map in the colvolution ply can be computed as:  
...
Figure \ref{fig:cpl} shows how predicting future steps is done, the future $x$ and the present$c$ are encoded into a compact distributed vector representations in a way that preserves both $x$ and $c$:  
